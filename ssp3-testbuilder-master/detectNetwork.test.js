 ...
 var expect = chai.expect;
  ...
  (function(prefix) {
    ...
    it('has a prefix of ' + prefix + ' and a length of 19', function(){
      expect(detectNetwork( prefix + '2345678901234567')).to.equal('Discover'); 
    });
  })(prefix)
}
});
  ...
  for (var i = 0; i < lengths.length; i++) {
    ...
    for (var j = 0; j < prefixes.length; j++) {
      ...
      while(prefix.length < len){
        ...
        prefix += ((prefix.length + 1) % 10).toString();
        
      }
        // cardNums.push(prefix);

      (function(prefix) {
        it('has a prefix of ' + prefix + ' and a length of ' + len, function(){
          expect(detectNetwork(prefix)).to.equal('Maestro');
        });
      })(prefix)
